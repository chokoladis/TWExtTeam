...
<div class="news-detail<?=$themeClass?>">
	...
	$arFilter_next = Array(
	 ...
	 $res_next = CIBlockElement::GetList(Array("ID"=>"DESC"), $arFilter_next, Array("CODE"));
	 if ($ar_fields = $res_next->GetNext())
		 echo "<a href='/news/$ar_fields[CODE]/' class='mr-2'>Предыдущая новость</a>";

	$arFilter_next = Array(
		"IBLOCK_ID"=>$arParams["IBLOCK_ID"],  
		">ID"=>$arResult["ID"],
	);
	$res_next = CIBlockElement::GetList(Array("ID"=>"ASC"), $arFilter_next, Array("CODE"));
	if ($ar_fields = $res_next->GetNext())
		echo "<a href='/news/$ar_fields[CODE]/'>Следующая новость</a>";
	?>
</div>
